...
def first_setup():
    ...
    local("mkvirtualenv funds_gather_moneydance_extension")

    # pip install packages
    with prefix('workon funds_gather_moneydance_extension'):
        local("pip install pytest")

def build_windows_dist():
    if os.name == 'nt':
        # Call the pyinstaller
        local("python ../pyinstaller/pyinstaller.py funds_gather_moneydance_extension_windows.spec --onefile")
...
def run_tests():
    ...
    with lcd(test_dir):
        ...
        t = local("py.test --cov-config .coveragerc --cov=funds_gather_moneydance_extension --cov-report=term --cov-report=html", capture=False)
...
def push_docs():
    ...
    """
    githubpages = "/Users/virantha/dev/githubdocs/funds_gather_moneydance_extension"
    ...
    with lcd("docs"):
        print("Running sphinx in docs/ and building to ~/dev/githubpages/funds_gather_moneydance_extension")
        local("make clean")
        local("make html")
